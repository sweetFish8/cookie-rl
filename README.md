...
### 転生(Ascension)は短ホライズンでは「罠」(2〜3ゲーム日、log10 累計焼成)

| ホライズン | 素朴な転生ヒューリスティック | ホライズン認識(残り<7日は転生しない) |
|---|---|---|
| 2ゲーム日 | 18.90 | **21.43** |
| 3ゲーム日 | 18.90 | **22.33** |

転生は全クッキーをリセットし、+1%/チップのプレステージ倍率は積み直しに長時間かかる。実測では **≤3ゲーム日では「転生しない」方が 2〜3.5 log10(数百〜数千倍)多く焼ける**。転生は本質的に最適停止問題で、報われるのは数週間スケールのプレイのみ。素朴な「倍増できたら転生」ヒューリスティックはこの領域で有害なので、貪欲を**残り時間が回収に足りる時だけ転生する**よう修正した(`ascend_min_remaining_days`)。RL は「今は転生するな」を学習でき、素朴な貪欲(18.9)を上回れる本物の隙になっている。
