...
| PPO (BC+微調整) | 11.05 ± 0.13 | *(調整中)* |

要点: 短ホライズンでは全方策が横並び。1ゲーム日では payback-period 貪欲が強力で、ランダムを約1000倍上回る。behavior cloning はその貪欲をニューラルネット方策として概ね再現できる(~15.0)。PPO 微調整で貪欲を明確に超えるところまでは至っておらず、継続課題(下記)。
...
- **PPO 微調整が BC 方策を初期に劣化させる。** BC は方策ネットのみ学習し価値関数(critic)が未校正なため、PPO の初期更新が誤った advantage で良い方策を壊す(BC→RL 転移の典型問題)。critic のみの事前学習、より小さい学習率、BC への KL 正則化などで貪欲超えを狙うのが次の一手。
- 転生(Ascension)を跨ぐ複数ゲーム日ホライズンでの長期戦略、ゴールデンクッキーの能動的活用(現状は自動ポップ)、グランマポカリプスの判断は未対応。
